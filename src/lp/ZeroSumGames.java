...
public class ZeroSumGames extends Strategy {
    ...
    public void execute() throws InterruptedException {
        ...
        while (true) {
            ...
            while (!playComplete) {
                ...
                game.showGame();
                this.zeroSumGames(U1, U2, labelsP1, labelsP2, myStrategy);
                try {
    ...
    public void zeroSumGames(int[][] M1, int[][] M2, String[] labelsP1, String[] labelsP2, PlayStrategy myStrategy) {
        int rows = M1.length;
        ...
        // Player2
        double[] c2 = new double[cols + 1];
        c2[c2.length - 1] = 1.0;

        double[] b2 = new double[rows + 1];
        b2[b2.length - 1] = 1.0;

        double[] lb2 = new double[cols + 1];
        lb2[lb2.length - 1] = getLowerBound(M1);

        double[][] A2 = new double[rows + 1][cols + 1];
        ...
        LinearProgram lp2 = new LinearProgram(c);
        lp2.setMinProblem(true);

        for (int i = 0; i < b2.length - 1; i++) {
            lp2.addConstraint(new LinearSmallerThanEqualsConstraint(A2[i], b2[i], "c" + i));
        }
        lp2.addConstraint(new LinearEqualsConstraint(A2[b2.length - 1], b2[b2.length - 1], "c" + (b2.length - 1)));
        lp2.setLowerbound(lb2);
        showLP(lp2);
        double[] res2 = solveLP(lp2);
        showSolution(lp2, res2);

        System.out.println("RESULTS P1:");
        for (int i = 0; i < res.length - 1; i++) {
            System.out.println(labelsP1[i] + ": " + res[i]);
        }
        System.out.println("RESULTS P2:");
        for (int i = 0; i < res2.length - 1; i++) {
            System.out.println(labelsP2[i] + ": " + res2[i]);
        }
